...
def enhancedFeatureExtractorDigit(datum):
    ...
    The following few features were used:

    1. 3 by 3 filters along with a padding of 5 were used.
    2. Total number of features which have 2, 3 or 4 shaded neighbors
    ##
    """
    features =  basicFeatureExtractorDigit(datum)

    "*** YOUR CODE HERE ***"
    # Number of features shaded
    xstride = 3; ystride = 3
    filterx = [x for x in range(5, DIGIT_DATUM_WIDTH-5) if not (x+1)%xstride]
    filtery = [y for y in range(5, DIGIT_DATUM_HEIGHT-5) if not (y+1)%ystride]

    filters = [(x, y) for x in filterx for y in filtery]

    for idx, filter in enumerate(filters):
        featureName = 'filter' + str(idx)
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) > 0:
                for idx, filter in enumerate(filters):
                    if (x<=filter[0]) and (x+xstride>filter[0]) and (y<=filter[1]) and (y+ystride>filter[1]):
                        featureName = 'filter' + str(idx)
                        features[featureName] += float(1)/(xstride*ystride)

    for x in range(1, DIGIT_DATUM_WIDTH-1):
        for y in range(1, DIGIT_DATUM_HEIGHT-1):
            if datum.getPixel(x, y) > 0:
                offset = 1
                neighbors = [(x+offset, y), (x, y+offset),
                             (x-offset, y), (x, y-offset)]
                shadedNeighbors = sum([1 for neighbor in neighbors if datum.getPixel(neighbor[0], neighbor[1])>0])
                if shadedNeighbors == 2:
                    features['2ShadedNeighbor'] += float(1)/(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT)
                elif shadedNeighbors == 3:
                    features['3ShadedNeighbor'] += float(1)/(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT)
                elif shadedNeighbors == 4:
                    features['4ShadedNeighbor'] += float(1)/(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT)
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    if features['remainingFood']:
        features['closestFoodDist'] = min([util.manhattanDistance(successor.getPacmanPosition(), (x, y)) for x in range(xrange) for y in range(yrange) if successor.hasFood(x, y)])

    # Capsule features
    features['remainingCapsule'] = len(successor.getCapsules())
    if features['remainingCapsule']:
        features['closestCapsuleDist'] = min([util.manhattanDistance(successor.getPacmanPosition(), capsulePos) for capsulePos in successor.getCapsules()])
