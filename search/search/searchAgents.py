...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    def corner_to_corner(state):
        current_pos, corners_not_visited = state
        total_dist = 0
        for _ in range(len(corners_not_visited)):
            dist_tuple = [(corner, util.manhattanDistance(current_pos, corner)) for corner in corners_not_visited]
            # Go to the nearest corner
            current_pos, dist = sorted(dist_tuple, key=lambda tup: tup[1])[0]

            # Update step - remove corners that was recently visited, and append total manhattan distance
            corners_not_visited = [corner for corner in list(corners_not_visited) if corner != current_pos]
            total_dist += dist

        return total_dist

    return corner_to_corner(state)
...
def foodHeuristic(state, problem):
    ...
    """
    "*** YOUR CODE HERE ***"
    current_pos, foodGrid = state; foodList = foodGrid.asList()
    list_of_tuples = []
    ...
    total_dist = (min([util.manhattanDistance(current_pos, furthest_foods[0]), util.manhattanDistance(current_pos, furthest_foods[1])])
                  + furthest_foods[2])
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        if (x, y) in self.food.asList(): return True
