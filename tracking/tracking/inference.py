...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: allPossible[self.getJailPosition()] = 1.0
        else:
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        perPositionParticles = self.numParticles/len(self.legalPositions)
        for p in self.legalPositions:
            self.particles += [p] * perPositionParticles
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if not self.getBeliefDistribution(): self.initializeUniformly(gameState)
        beliefDistribution = self.getBeliefDistribution()

        weightedDistribution = util.Counter()
        if noisyDistance is None: weightedDistribution[self.getJailPosition()] = 1.0
        else:
            for particle in self.particles:
                trueDistance = util.manhattanDistance(particle, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    weightedDistribution[particle] = (emissionModel[trueDistance] * beliefDistribution[particle])

        # Resampling step
        if not weightedDistribution.totalCount():
            self.initializeUniformly(gameState)
            weightedDistribution = self.getBeliefDistribution()

        self.particles = util.nSample(weightedDistribution.values(), weightedDistribution.keys(), self.numParticles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for pos in itertools.product(self.legalPositions, repeat=self.numGhosts):
            self.particles += [tuple(pos)] * perPositionParticles
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            newParticle = \
                [util.sample(getPositionDistributionForGhost(
                    setGhostPositions(gameState, list(oldParticle)), idx, self.ghostAgents[idx])
                 ) for idx in range(self.numGhosts)]
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        beliefDistribution = util.Counter()
        for ghostPosTuple in self.particles:
            beliefDistribution[ghostPosTuple] += 1
